<div class="container mx-auto px-4">
    <h1 class="text-2xl font-bold mb-4 text-pink-600">Daftar Pegawai</h1>
    
    <div class="flex justify-start mb-4">
        <a href="{{ route('pegawai.create') }}" class="bg-pink-600 text-white px-4 py-2 rounded font-semibold text-sm hover:bg-pink-700">
            + Tambah Pegawai
        </a>
    </div>
        ...
        <table class="min-w-full border-collapse mt-4 text-center">
            <thead>
                <tr class="bg-pink-700 text-white">
                    ...
                    <th class="py-3 px-4 border border-pink-600">NIP</th>
                    <th class="py-3 px-4 border border-pink-600">Nama</th>
                    <th class="py-3 px-4 border border-pink-600">Unit Kerja</th>
            ...
            <tbody>
                @foreach ($pegawais as $index => $row)
                    <tr class="{{ $index % 2 === 0 ? 'bg-pink-100' : 'bg-pink-200' }} text-pink-900">
                        <td class="py-2 px-4 border border-pink-300">{{ $row->id }}</td>
                        <td class="py-2 px-4 border border-pink-300">{{ $row->nip }}</td>
                        <td class="py-2 px-4 border border-pink-300 text-left">{{ $row->nama }}</td>
                        <td class="py-2 px-4 border border-pink-300">{{ $row->unitKerja?->nama ?? '-' }}</td>
